...
p <- ggplot(subset_stb_survey, aes(x = Chemical, fill = State)) +
  ...
  labs(title = "Use Frequency of Top 10 Chemicals in various State", 
...
*The chart visualizes the use frequency of the top 10 chemicals across various states.California seems to have the highest use frequency for most of the listed chemicals, particularly for malathion, flumioxazin, and fludioxonil.Florida demonstrates a moderate use frequency. It uses some chemicals, like fenhexamid and cyprodinil, more frequently than other states but uses others, like malathion, less often.Oregon's usage appears to be relatively lower than that of California and Florida for most chemicals. However, for some chemicals, like bifenthrin, its use frequency is comparable to Florida.Washington has the least use frequency for nearly all of the listed chemicals. The usage seems to be minimal and consistent across the chemicals. Malathion stands out as the chemical with the highest use frequency, especially in California.Abamectin appears to be the least frequently used chemical among the four states..*
